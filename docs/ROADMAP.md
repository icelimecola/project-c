...
当前已经完成自动捕获文本剪贴板、监听开关、后端搜索、SQLite FTS 和全局快捷键。接下来重点是把“能打开”变成“打开后能快速完成动作”：搜索、选择、复制、隐藏回后台。
...
- 窗口被快捷键唤起后，搜索框自动获得焦点。
- 支持上下键移动当前 clip。
- 支持 Enter 将当前 clip 写回系统剪贴板。
- 支持 Escape 隐藏窗口或清空搜索。
- 保持当前列表、详情和 folder 状态稳定。
...
### Add Clipboard Writeback

目标：让搜索结果可以真正回写到系统剪贴板，完成“找回历史内容”的闭环。

计划工作：

- 新增 Rust command：copy clip content to clipboard。
- Enter 或按钮触发写回。
- 更新 `last_used_at`。
- 写回后避免监听循环把同一内容重复捕获。
